...
# Docker Desktop bind mounts may not support ACLs. Keep startup non-fatal there.
if ! setfacl -R -m u:$DOCKER_USER_ID:rwx /var/www/html/ 2>/dev/null; then
    echo "setfacl is not supported on this filesystem, skipping ACL setup."
fi

if ! setfacl -R -d -m u:$DOCKER_USER_ID:rwx /var/www/html/ 2>/dev/null; then
    echo "Default ACLs are not supported on this filesystem, skipping ACL setup."
fi
