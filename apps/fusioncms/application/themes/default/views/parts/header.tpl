...
{capture assign=feature_menu}
	<li class="nav-item">
		<a href="{$url}howtoplay" class="nav-link {if $CI->router->fetch_module() == 'howtoplay'}active{/if}" title="How to Play">How to Play</a>
	</li>
	<li class="nav-item">
		<a href="{$url}server" class="nav-link {if $CI->router->fetch_module() == 'server'}active{/if}" title="Server">Server</a>
	</li>
	<li class="nav-item">
		<a href="{$url}rankings" class="nav-link {if $CI->router->fetch_module() == 'rankings'}active{/if}" title="Rankings">Rankings</a>
	</li>
	<li class="nav-item">
		<a href="{$url}playerbots" class="nav-link {if $CI->router->fetch_module() == 'playerbots'}active{/if}" title="Playerbots">Playerbots</a>
	</li>
{/capture}
...
<div id="mobileMenu" class="offcanvas offcanvas-end" aria-labelledby="mobileMenuLabel">
	...
	<div class="offcanvas-body">
		...
		<nav class="navbar">
			<!-- Nav.Start -->
			<ul class="navbar-nav">{$menus.top}{$feature_menu}</ul>
			<!-- Nav.End -->
		</nav>
		<!-- Navbar.End -->
	</div>
			...
			<div class="col-sm-12">
				...
				<nav class="navbar navbar-expand-xl">
					...
					<div class="navbar-collapse collapse">
						<!-- Nav.Start -->
						<ul class="navbar-nav ms-auto">{$menus.top}{$feature_menu}{$user_menu}</ul>
						<!-- Nav.End -->
					</div>
					<!-- Collapse.End -->
				</nav>
