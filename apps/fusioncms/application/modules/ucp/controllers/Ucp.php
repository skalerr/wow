...
class Ucp extends MX_Controller
{
    ...
    public function index()
    {
        ...
        foreach ($this->realms->getRealms() as $realm) {
            $characters = $realm->getCharacters()->getCharactersByAccount();
            $characterSummary['realmSpread'][$realm->getId()] = [
                'name' => $realm->getName(),
            ...
            foreach ($characters as $character) {
                $characterSummary['totalGold'] += (int) $character['money'];

                if (!empty($character['online'])) {
                    $characterSummary['onlineCharacters']++;
                }

                if ((int) $character['level'] >= $characterSummary['highestLevel']) {
                    $characterSummary['highestLevel'] = (int) $character['level'];
                    $characterSummary['highestCharacter'] = [
                        'name' => $character['name'],
                        'realm' => $realm->getName(),
                    ];
                }
        ...
        $data['dashboard'] = $characterSummary;
        $data['quick_links'] = [
            ['title' => 'Server', 'link' => $this->template->page_url . 'server'],
            ['title' => 'Rankings', 'link' => $this->template->page_url . 'rankings'],
            ['title' => 'How to Play', 'link' => $this->template->page_url . 'howtoplay'],
            ['title' => 'Playerbots', 'link' => $this->template->page_url . 'playerbots'],
        ];
