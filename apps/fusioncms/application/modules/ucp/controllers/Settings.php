...
class Settings extends MX_Controller
{
    ...
    public function index()
    {
        ...
        clientLang("invalid_expansion", "ucp");

        $canChangeExpansion = hasPermission("canChangeExpansion");
        $expansions = [];

        if ($canChangeExpansion) {
            $maxExpansion = (int) $this->config->item('max_expansion');

            foreach ($this->realms->getExpansions() as $id => $name) {
                if ((int) $id <= $maxExpansion) {
                    $expansions[$id] = $name;
                }
            }
        }
        ...
        $settings_data = array(
            ...
            'currentExpansion' => $this->external_account_model->getExpansion(),
            'canChangeExpansion' => $canChangeExpansion,
    ...
    public function submitInfo()
    {
        ...
        if (hasPermission("canChangeExpansion") && $expansion !== null && $expansion !== '') {
            $maxExpansion = (int) $this->config->item('max_expansion');

            if (!is_numeric($expansion) || (int) $expansion < 0 || (int) $expansion > $maxExpansion) {
                die(lang("invalid_expansion", "ucp"));
            }

            $this->user->setExpansion((int) $expansion);
        }
