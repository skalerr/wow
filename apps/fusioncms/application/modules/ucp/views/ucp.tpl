<div class="container">
	<div class="row">
		...
		<div class="col-lg-8 py-lg-5 pb-5 pb-lg-0">
			<div class="ucp-dashboard-grid mb-4">
				<div class="ucp-dashboard-card">
					<span class="label">Characters</span>
					<strong>{$characters}</strong>
				</div>
				<div class="ucp-dashboard-card">
					<span class="label">Online now</span>
					<strong>{$dashboard.onlineCharacters}</strong>
				</div>
				<div class="ucp-dashboard-card">
					<span class="label">Highest level</span>
					<strong>{if $dashboard.highestCharacter}{$dashboard.highestLevel} · {$dashboard.highestCharacter.name}{else}0{/if}</strong>
				</div>
				<div class="ucp-dashboard-card">
					<span class="label">Quick links</span>
					<div class="ucp-dashboard-links">
						{foreach from=$quick_links item=item}
			...
			<div class="section-body">
				<table class="table table-borderless table-responsive user-table">
					<tbody class="text-break">
						<tr>
							<td><div class="user-table-icon"><i class="fa-duotone fa-user"></i></div> {lang("nickname", "ucp")}</td>
							<td>{$username}</td>
							<td class="text-end"><a href="{$url}ucp/settings">{lang("edit", "ucp")}</a></td>
						</tr>
						<tr>
							<td><div class="user-table-icon"><i class="fa-duotone fa-envelope"></i></div> {lang("email", "ucp")}</td>
							<td>{$email}</td>
							<td class="text-end"><a href="{$url}ucp/settings">{lang("edit", "ucp")}</a></td>
						</tr>
						<tr>
						...
						<tr>
							<td><div class="user-table-icon"><i class="fa-duotone fa-user-lock"></i></div> {lang("account_status", "ucp")}</td>
							<td colspan="2">{$status}</td>
						</tr>
						<tr>
							<td><div class="user-table-icon"><i class="fa-duotone fa-calendar"></i></div> {lang("member_since", "ucp")}</td>
							<td colspan="2">{$register_date}</td>
						</tr>
						<tr>
							<td><div class="user-table-icon"><i class="fa-duotone fa-user-shield"></i></div> {lang("account_rank", "ucp")}</td>
						...
						<tr>
							<td><div class="user-table-icon"><i class="fa-duotone fa-location-dot"></i></div> {lang("location", "ucp")}</td>
							<td>{$location}</td>
							<td class="text-end"><a href="{$url}ucp/settings">{lang("edit", "ucp")}</a></td>
						</tr>
						...
						<tr class="user-points">
							<td><div class="vote-points user-table-icon"><i class="fa-duotone fa-coins"></i></div> {lang("voting_points", "main")}</td>
							<td>{$vp}</td>
						...
						<tr  class="user-points">
							<td><div class="donation-points user-table-icon"><i class="fa-duotone fa-coins"></i></div> {lang("donation_points", "main")}</td>
							<td>{$dp}</td>
									...
									{foreach from=$realm->getCharacters()->getCharactersByAccount() item=character}
										<tr>
											...
											<td class="col-4 user-points">
												{if $money}
													{if {$money["gold"]}}
														<span class="moneygold">{$money["gold"]}</span>
													{/if}
													{if {$money["silver"]}}
														<span class="moneysilver">{$money["silver"]}</span>
													{/if}
													<span class="moneycopper">{$money["copper"]}</span>
												{else}
													<span class="moneycopper">0</span>
												{/if}
											</td>
			...
			<div class="section-header">Realm spread</div>
			<div class="section-body">
				<div class="ucp-dashboard-grid compact">
					{foreach from=$dashboard.realmSpread item=realm}
						<div class="ucp-dashboard-card">
							<span class="label">{$realm.name}</span>
							<strong>{$realm.count}</strong>
						</div>
					{/foreach}
				</div>
			</div>
