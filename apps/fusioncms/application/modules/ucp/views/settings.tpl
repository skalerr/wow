			...
			<div class="section-body">
				...
				<form onSubmit="Settings.submitInfo(); return false" id="settings_info" class="page_form">
					...
					{if $canChangeExpansion}
						<div class="form-group row">
							<label class="col-sm-3 col-form-label" for="expansion_field">{lang("expansion", "ucp")}</label>
							<div class="col-sm-9">
								<select class="form-select" aria-label="{lang("expansion", "ucp")}" name="expansion_field" id="expansion_field">
									{foreach from=$expansions key=id item=expansion}
										<option value="{$id}" {if $currentExpansion == $id}selected="selected"{/if}>{$expansion}</option>
									{/foreach}
								</select>
							</div>
						</div>
